...
- `mobile-security` — Secure storage, tokens, health-data privacy
- `mobile-push-notifications` — APNs/FCM, Expo Push API
...
- `fedramp-compliance` — cdk-nag, NIST 800-53 controls, FedRAMP Moderate baselines
...
> Hooks target Linux/WSL2 (GNU grep/sed/date). macOS is untested — several scripts use `grep -P`, which BSD grep lacks.
...
# Decision outputs (PreToolUse). stdout must be ONLY valid JSON — always
# build it with jq so special characters can't break parsing:
#   exit 0 with no output → allow (normal permission flow)
#   exit 2, reason on stderr → deny
#   jq -n --arg r "why" '{"hookSpecificOutput":{"hookEventName":"PreToolUse",
#     "permissionDecision":"deny","permissionDecisionReason":$r}}'   → deny
#   ...same with "permissionDecision":"ask" → prompt the user
#
# PostToolUse: stdout on exit 0 is NOT shown to Claude. To give Claude
# feedback (lint failures, reminders), print to stderr and exit 2.
```

2. Make it executable: `chmod +x hooks/my-hook.sh`
3. Add a row to the **Hooks** table in this README.
4. Register the hook in the appropriate `settings.json`. For global hooks, add an entry to `config/settings.json`; for project hooks, add it to the settings generated by `project-init.sh`.

**Registration rules (these are easy to get wrong):**
- The `matcher` is a regex against the **tool name only** (e.g. `"Write|Edit"`). File-path filtering happens *inside* the hook script by reading `tool_input.file_path` from stdin — `Write(*.py)`-style globs are permission-rule syntax and never match as hook matchers.
- `timeout` is in **seconds**, not milliseconds.

**Available events:**
- `PreToolUse` — runs before a tool call (can deny or ask)
- `PostToolUse` — runs after a tool call completes
- `UserPromptSubmit` — runs on each user prompt (stdout is added to context)
- `SessionStart` — runs on session start (stdout is added to context)
- `Notification` / `Stop` — notifications and end-of-response
